...
import sap.s4hana.api_sd_incoterms_srv.mock as _;
...
function testListA_SalesOrder() returns error? {
    CollectionOfA_IncotermsClassificationWrapper wrapper = check s4HanaClient->listA_IncotermsClassifications();
    test:assertTrue(wrapper.d?.results !is (), "Expected to receive atleast one inco terms results");
}
