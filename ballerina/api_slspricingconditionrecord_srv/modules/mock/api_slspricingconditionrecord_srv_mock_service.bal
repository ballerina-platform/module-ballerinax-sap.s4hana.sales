...
service /sap/opu/odata/sap/API_SLSPRICINGCONDITIONRECORD_SRV on ep0 {
    ...
    # http:Response (Error)
    resource function get A_SlsPrcgConditionRecord(int? \$top, int? \$skip, string? \$filter, "allpages"|"none"? \$inlinecount, A_SlsPrcgConditionRecordOrderByOptions? \$orderby, A_SlsPrcgConditionRecordSelectOptions? \$select, A_SlsPrcgConditionRecordExpandOptions? \$expand) returns CollectionOfA_SlsPrcgConditionRecordWrapper|http:Response {
        return {
            d: {
                results: [
                    {
    ...
    # http:Response (Error)
    resource function post A_SlsPrcgConditionRecord(@http:Payload CreateA_SlsPrcgConditionRecord payload) returns A_SlsPrcgConditionRecordWrapper|http:Response {
        return {
            d: {
                ConditionRecord: payload.ConditionRecord,
                ConditionSequentialNumber: payload.ConditionSequentialNumber
            }
        };
    }
}
