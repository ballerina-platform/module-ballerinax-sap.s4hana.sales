...
import sap.s4hana.ce_salesorder_0001.mock as _;
...
function testListA_SalesOrder() returns error? {
    CollectionOfSalesOrder listSalesOrders = check s4HanaClient->listSalesOrders();
    test:assertTrue(listSalesOrders.value !is (), "The sales order is expected to be non-empty.");
}

@test:Config {
}
function testCreateSalesOrder() returns error? {
    SalesOrder|error salesOrder = s4HanaClient->createSalesOrder({
        SalesOrder: "1",
    ...
    test:assertTrue(salesOrder is error, "Error occurred while creating the sales order.");
    error e = <error>salesOrder;
    test:assertEquals(e.detail()["statusCode"], 400, "The status code is expected to be 400. External Number is not allowed");
}
