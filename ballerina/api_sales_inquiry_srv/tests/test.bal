...
import sap.s4hana.api_sales_inquiry_srv.mock as _;
...
function testListA_SalesInquiries() returns error? {
    CollectionOfA_SalesInquiryWrapper listA_SalesOrders = check s4HanaClient->listA_SalesInquiries();
    test:assertTrue(listA_SalesOrders.d?.results !is (), "The sales inquiry is expected to be non-empty.");
}
