...
service /sap/opu/odata/sap/API_SALES_INQUIRY_SRV on ep0 {
    ...
    # http:Response (Error)
    resource function get A_SalesInquiry(int? \$top, int? \$skip, string? \$filter, "allpages"|"none"? \$inlinecount, A_SalesInquiryOrderByOptions? \$orderby, A_SalesInquirySelectOptions? \$select, A_SalesInquiryExpandOptions? \$expand) returns CollectionOfA_SalesInquiryWrapper|http:Response {
        return {
            d: {
                results: [
                    {
                        SalesInquiry: "1000000"
                    }
                ]
            }
        };
