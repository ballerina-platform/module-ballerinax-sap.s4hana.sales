...
service /sap/opu/odata/sap/API_SALES_QUOTATION_SRV on ep0 {
    ...
    # http:Response (Error)
    resource function get A_SalesQuotation(int? \$top, int? \$skip, string? \$filter, "allpages"|"none"? \$inlinecount, A_SalesQuotationOrderByOptions? \$orderby, A_SalesQuotationSelectOptions? \$select, A_SalesQuotationExpandOptions? \$expand) returns CollectionOfA_SalesQuotationWrapper|http:Response {
        return {
            d: {
                results: [
                    {
                        SalesQuotation: "123456"
                    }
                ]
            }
        };
    ...
    # http:Response (Error)
    resource function post A_SalesQuotation(@http:Payload CreateA_SalesQuotation payload) returns A_SalesQuotationWrapper|http:Response {
        return {
            d: {
                SalesQuotation: payload.SalesQuotation
            }
        };
    }
}
